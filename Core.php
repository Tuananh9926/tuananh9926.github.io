...
require $_SERVER['DOCUMENT_ROOT'].'/HIEUVIP22CONFIG/hieuvip22database.php'; // website config 
...
class System {
   ...
   public function connect_db() {
    ...
    public function settings($option) {
        ...
        $_rut_kc = mysqli_fetch_assoc(mysqli_query($this->connect_db(), "SELECT * FROM `settings` WHERE `key`='rut_kc'"));
        $_web_brick = mysqli_fetch_assoc(mysqli_query($this->connect_db(), "SELECT * FROM `settings` WHERE `key`='web_brick'"));
        $_cardv3 = mysqli_fetch_assoc(mysqli_query($this->connect_db(), "SELECT * FROM `settings` WHERE `key`='web_cardv3'"));
        ...
        switch ($option) {
            ...
            case 'napthe':
                ...
                break;
            case 'cardv3':
                return json_decode($_cardv3['value'], true);
                break;
            case 'rut_kc':
                return json_decode($_rut_kc['value'], true);
                break;
            case 'brick':
...
//Phân trang v2 - K6VN TEAM, do v1 nhìn xấu quá :v
    /*
    * $url: Đường dẫn phân trang
    * $total: Tổng số kết quả
    * $limit: Số kết quả mỗi trang
    * $current: Trang hiện tại
    */
    function pag($url, $total, $limit, $current = 1){
        $return = '<div class="text-center mb-3"><div class="btn-group mr-2" role="group">';
        $total_page = ceil($total/$limit);
        
        if($current > 3) $return .= '<a type="button" class="btn btn-secondary" href="' . $url . '/page=1">«</a>';
        if($current > 2) $return .= '<a type="button" class="btn btn-secondary" href="' . $url . '/page='.($current-2).'">'.($current-2).'</a>';
        if($current > 1) $return .= '<a type="button" class="btn btn-secondary" href="' . $url . '/page='.($current-1).'">'.($current-1).'</a>';
        $return .= '<button type="button" class="btn btn-info disabled">'.$current.'</button>';
        if($current < $total_page) $return .= '<a type="button" class="btn btn-secondary" href="' . $url . '/page='.($current+1).'">'.($current+1).'</a>';
        if($current < $total_page - 1) $return .= '<a type="button" class="btn btn-secondary" href="' . $url . '/page='.($current+2).'">'.($current+2).'</a>';
        if($current < $total_page - 2) $return .= '<a type="button" class="btn btn-secondary" href="' . $url . '/page='.$total_page.'">»</a>';
